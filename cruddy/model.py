...
def model_builder():
    ...
    for row in table:
        ...
        try:
            ...
            pass
        shutil.copy("../static/assets/vintage-illustration-of-a-brain.jpg", "../volumes/uploads")
        # add some notes with default image
        try:
            '''add a few 1 to 4 notes per user'''
            for num in range(randrange(1, 4)):
                note = "#### " + row.name + " note " + str(num) + ". \n Generated by test data."
                row.notes.append(Notes(userID=row.userID, note=note, image='vintage-illustration-of-a-brain.jpg'))
